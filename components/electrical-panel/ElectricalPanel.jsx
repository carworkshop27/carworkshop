...
export default function ElectricalPanel({ onConfirmItem }) {
  ...
  useEffect(() => {
    const loadParts = async () => {
      ...
      if (error) {
        console.error("Failed to load electrical parts:", error);
        return;
      }
  ...
  const handleDeletePart = async (partId, partName) => {
    ...
    if (error) {
      console.error("Failed to remove electrical part:", error);
  ...
  const handleAddPart = async () => {
    ...
    if (error) {
      console.error("Failed to add electrical part:", error);
      alert("Failed to add the new part.");
      return;
    }
  ...
  return (
    <section className="mt-6 rounded-2xl border border-slate-200 bg-white shadow-sm overflow-visible">
      <div className="px-6 py-5 border-b border-slate-200">
        <h2 className="text-xl font-bold text-slate-900">Electrical Panel</h2>

        <p className="mt-1 text-sm text-slate-500">
          Electrical inspection items and repair costs
        </p>
      </div>
