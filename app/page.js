...
function DetailedCarModel15Panels({ panels, selectedPanelId, onSelectPanel }) {
  const groupRef = useRef();
  
  const getMaterialProps = (panelId) => {
    const p = (panels || []).find(item => item.id === panelId);
    const status = p?.status || 'ok';
    const isSelected = selectedPanelId === panelId;
    const info = getDamageInfo(status);
    
    let color = info.color;
    if (status === 'ok' && isSelected) color = '#2563eb'; 
    
    return { color, metalness: 0.6, roughness: 0.2 };
  };

  return (
    <group ref={groupRef} position={[0, -0.4, 0]}>
      <mesh position={[0, 0.42, 1.45]} rotation={[-0.1, 0, 0]} onClick={(e) => { e.stopPropagation(); onSelectPanel('hood'); }}>
        <boxGeometry args={[1.5, 0.15, 1.2]} /><meshStandardMaterial {...getMaterialProps('hood')} />
      </mesh>
      <mesh position={[0, 0.15, 2.2]} onClick={(e) => { e.stopPropagation(); onSelectPanel('front-bumper'); }}>
        <boxGeometry args={[1.6, 0.35, 0.35]} /><meshStandardMaterial {...getMaterialProps('front-bumper')} />
      </mesh>
      <mesh position={[0, 0.9, -0.1]} onClick={(e) => { e.stopPropagation(); onSelectPanel('roof'); }}>
        <boxGeometry args={[1.35, 0.12, 1.3]} /><meshStandardMaterial {...getMaterialProps('roof')} />
      </mesh>
      <mesh position={[0, 0.45, -1.6]} rotation={[0.08, 0, 0]} onClick={(e) => { e.stopPropagation(); onSelectPanel('trunk'); }}>
        <boxGeometry args={[1.45, 0.15, 0.9]} /><meshStandardMaterial {...getMaterialProps('trunk')} />
      </mesh>
      <mesh position={[0, 0.18, -2.1]} onClick={(e) => { e.stopPropagation(); onSelectPanel('rear-bumper'); }}>
        <boxGeometry args={[1.6, 0.38, 0.3]} /><meshStandardMaterial {...getMaterialProps('rear-bumper')} />
      </mesh>
      <mesh position={[-0.82, 0.35, 1.45]} onClick={(e) => { e.stopPropagation(); onSelectPanel('front-left-fender'); }}>
        <boxGeometry args={[0.12, 0.4, 1.1]} /><meshStandardMaterial {...getMaterialProps('front-left-fender')} />
      </mesh>
      <mesh position={[0.82, 0.35, 1.45]} onClick={(e) => { e.stopPropagation(); onSelectPanel('front-right-fender'); }}>
        <boxGeometry args={[0.12, 0.4, 1.1]} /><meshStandardMaterial {...getMaterialProps('front-right-fender')} />
      </mesh>
      <mesh position={[-0.82, 0.45, -1.35]} onClick={(e) => { e.stopPropagation(); onSelectPanel('rear-left-fender'); }}>
        <boxGeometry args={[0.12, 0.45, 1.1]} /><meshStandardMaterial {...getMaterialProps('rear-left-fender')} />
      </mesh>
      <mesh position={[0.82, 0.45, -1.35]} onClick={(e) => { e.stopPropagation(); onSelectPanel('rear-right-fender'); }}>
        <boxGeometry args={[0.12, 0.45, 1.1]} /><meshStandardMaterial {...getMaterialProps('rear-right-fender')} />
      </mesh>
      <mesh position={[-0.78, 0.45, 0.45]} onClick={(e) => { e.stopPropagation(); onSelectPanel('front-left-door'); }}>
        <boxGeometry args={[0.1, 0.6, 0.85]} /><meshStandardMaterial {...getMaterialProps('front-left-door')} />
      </mesh>
      <mesh position={[0.78, 0.45, 0.45]} onClick={(e) => { e.stopPropagation(); onSelectPanel('front-right-door'); }}>
        <boxGeometry args={[0.1, 0.6, 0.85]} /><meshStandardMaterial {...getMaterialProps('front-right-door')} />
      </mesh>
      <mesh position={[-0.78, 0.45, -0.42]} onClick={(e) => { e.stopPropagation(); onSelectPanel('rear-left-door'); }}>
        <boxGeometry args={[0.1, 0.6, 0.85]} /><meshStandardMaterial {...getMaterialProps('rear-left-door')} />
      </mesh>
      <mesh position={[0.78, 0.45, -0.42]} onClick={(e) => { e.stopPropagation(); onSelectPanel('rear-right-door'); }}>
        <boxGeometry args={[0.1, 0.6, 0.85]} /><meshStandardMaterial {...getMaterialProps('rear-right-door')} />
      </mesh>
      <mesh position={[-0.78, 0.08, 0.02]} onClick={(e) => { e.stopPropagation(); onSelectPanel('left-skirt'); }}>
        <boxGeometry args={[0.12, 0.15, 1.7]} /><meshStandardMaterial {...getMaterialProps('left-skirt')} />
      </mesh>
      <mesh position={[0.78, 0.08, 0.02]} onClick={(e) => { e.stopPropagation(); onSelectPanel('right-skirt'); }}>
        <boxGeometry args={[0.12, 0.15, 1.7]} /><meshStandardMaterial {...getMaterialProps('right-skirt')} />
      </mesh>
      <mesh position={[-0.85, 0, 1.3]} rotation={[0, 0, Math.PI / 2]}><cylinderGeometry args={[0.35, 0.35, 0.25, 32]} /><meshStandardMaterial color="#0f172a" roughness={0.8} /></mesh>
      <mesh position={[0.85, 0, 1.3]} rotation={[0, 0, Math.PI / 2]}><cylinderGeometry args={[0.35, 0.35, 0.25, 32]} /><meshStandardMaterial color="#0f172a" roughness={0.8} /></mesh>
      <mesh position={[-0.85, 0, -1.3]} rotation={[0, 0, Math.PI / 2]}><cylinderGeometry args={[0.35, 0.35, 0.25, 32]} /><meshStandardMaterial color="#0f172a" roughness={0.8} /></mesh>
      <mesh position={[0.85, 0, -1.3]} rotation={[0, 0, Math.PI / 2]}><cylinderGeometry args={[0.35, 0.35, 0.25, 32]} /><meshStandardMaterial color="#0f172a" roughness={0.8} /></mesh>
    </group>
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-700">
          <div className="bg-slate-950 text-white p-6 text-center border-b border-slate-800">
            ...
            <p className="text-xs text-slate-400 mt-1">Multi-User Authentication Portal</p>
          </div>

          <form onSubmit={handleLoginSubmit} className="p-6 space-y-4">
            <div>
              <label className="block text-xs font-black text-slate-700 mb-1">Username</label>
            ...
            <div>
              <label className="block text-xs font-black text-slate-700 mb-1">PIN / Password</label>
              <div className="relative">
                <KeyRound className="w-4 h-4 text-slate-400 absolute left-3 top-3" />
                <input 
                  type="password" required placeholder="Enter your PIN" 
                  value={loginInput.pin} onChange={(e) => setLoginInput({ ...loginInput, pin: e.target.value })} 
                  className="w-full pl-9 pr-3 py-2 text-sm font-bold text-slate-900 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" 
                />
              </div>
            </div>

            <button type="submit" className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs uppercase tracking-wider rounded-lg shadow transition-all">
              Sign In to Workshop
            </button>
          </form>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-300 shadow-sm overflow-hidden p-6">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-200">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-xl font-black text-slate-900">Inventory Catalog ({PAINT_CHEMICALS_INVENTORY.length} Items)</h3>
            ...
            <div className="overflow-x-auto max-h-[600px]">
              <table className="w-full text-left text-xs text-slate-700">
                ...
                <tbody className="divide-y divide-slate-200 font-bold">
                  {PAINT_CHEMICALS_INVENTORY.map((item, index) => (
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-bold text-base flex items-center space-x-2">
                  <span>360° Real 3D Inspector ({activeJob ? activeJob.id : 'Live Model'})</span>
                </h3>
              </div>
            </div>
          </div>
          <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 bg-slate-950 rounded-xl border border-slate-800 relative h-[480px] overflow-hidden cursor-grab active:cursor-grabbing">
              <Canvas camera={{ position: [3.5, 3.5, 5], fov: 45 }}>
                <ambientLight intensity={0.7} />
                <directionalLight position={[10, 10, 5]} intensity={1.2} />
                <directionalLight position={[-10, -10, -5]} intensity={0.4} />
                <Suspense fallback={null}>
                  <DetailedCarModel15Panels panels={panels} selectedPanelId={selectedPanelId} onSelectPanel={(id) => setSelectedPanelId(id)} />
                </Suspense>
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 flex flex-col justify-between">
              <div>
                <div className="border-b border-slate-200 pb-4 mb-4">
                  <span className="text-xs text-slate-400 uppercase font-semibold tracking-wider">Active Panel</span>
                  <h4 className="text-xl font-bold text-slate-900">{selectedPanel?.name}</h4>
                  
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    {Object.entries(DAMAGE_TYPES).map(([key, info]) => (
                      <button
                        key={key}
                        onClick={() => updatePanelDamage(selectedPanelId, key)}
                        className={`py-2 px-2 text-xs font-bold rounded-lg border transition-all ${
                          selectedPanel?.status === key ? `ring-2 ring-blue-500 shadow-md ${info.uiColor}` : 'bg-slate-50 text-slate-700 border-slate-200 hover:bg-slate-100'
                        }`}
                      >
                        {info.label} <br/><span className="text-[10px] font-extrabold">${info.cost}</span>
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-200">
                    <label className="block text-[11px] font-extrabold text-slate-700 mb-1 flex items-center gap-1">
                      <UserCheck className="w-3.5 h-3.5 text-indigo-600" /> Assign Panel Technician
                    </label>
                    <select 
                      value={selectedPanel?.assignedTech || ''} 
                      onChange={(e) => updatePanelTechnician(selectedPanelId, e.target.value)}
                      className="w-full bg-slate-50 text-xs font-bold text-slate-900 p-2 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-600"
                    >
                ...
                <div className="space-y-2 mb-4">
                  <span className="text-xs text-slate-500 uppercase font-bold tracking-wider block mb-2">Quick Navigation</span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 max-h-[140px] overflow-y-auto pr-1 border rounded-lg p-2 bg-slate-50">
                    {panels.map((p) => {
                      const tInfo = getDamageInfo(p.status);
                      return (
                        <button
                          key={p.id}
                          onClick={() => setSelectedPanelId(p.id)}
                          className={`px-2 py-1.5 rounded text-[11px] font-bold border transition-colors text-left truncate flex items-center justify-between ${
                            p.id === selectedPanelId ? 'ring-1 ring-slate-500 ' : ''
                          } ${tInfo.uiColor}`}
                        >
              ...
              <div className="space-y-3">
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 flex items-center justify-between">
                  <div className="flex items-center space-x-2 text-slate-800 font-bold text-xs">
                    <Activity className="w-4 h-4 text-blue-600" /><span>Total Invoice Estimate</span>
                  </div>
                  <strong className="text-slate-900 font-extrabold text-lg">
                    ${((panels || []).reduce((sum, p) => sum + getDamageInfo(p.status).cost, 0) + (activeJob?.parts || []).reduce((sum, pt) => sum + pt.price, 0))}
                  </strong>
                </div>
                <button onClick={handleDownloadPDF} disabled={!activeJob} className={`w-full py-3 px-4 rounded-lg text-xs font-bold border transition-colors shadow flex items-center justify-center space-x-2 ${activeJob ? 'bg-blue-600 hover:bg-blue-700 text-white border-blue-600' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>
                  <FileDown className="w-4 h-4" /><span>Download Sales Invoice (PDF)</span>
                </button>
              </div>
            </div>
          </div>
